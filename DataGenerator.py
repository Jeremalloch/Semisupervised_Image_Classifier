...
class DataGenerator:
    ...
    """
    def __init__(self, maxHammingSet, xDim=64, yDim=64, numChannels=3, numCrops=9, batchSize=32, meanTensor=None, stdTensor=None):
        """
...
def test():
    ...
    test_gen = DataGenerator(maxHammingSet=max_hamming_set, batchSize=batch_size, meanTensor=normalize_mean, stdTensor=normalize_std)
    ...
    print("Elapsed time {} for {} batches".format(end-start, num_epochs*(train_dataset.shape[0]//batch_size)))
    print("Time per batch: {} milliseconds".format(1000*(end-start)/(num_epochs*(train_dataset.shape[0]//batch_size))))
