...
def benchmark(loop_count=250):
    ...
    file_ = "resized/"
    file_paths = [file_+ "HOCO.jpg", file_ + "car_1.jpg", file_ + "car_2.jpg", file_ + "car_3.jpg"]

    images = np.array([Image.open(path) for path in file_paths], dtype=np.uint8)

    # Run benchmark for how long it takes to convert to create jigsaw from an image
    start_time = time.time()
    for _ in range(loop_count):
        for index in range(4):
            cropped_array = create_croppings(colour_channel_jitter(images[index]))
    end_time = time.time()
    del cropped_array
